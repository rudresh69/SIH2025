...
# Allow local dev origins for HTTP and WebSocket
allowed_origins = [
    "http://localhost:5173",
    "http://127.0.0.1:5173",
    "http://localhost:3000",
...
async def get_sensor_groups():
    return JSONResponse(SENSOR_GROUPS)
...
async def websocket_live(ws: WebSocket):
    # Accept WebSocket from any allowed origin (for dev)
    await ws.accept()
    try:
        while True:
            readings = sensors.get_all_readings()
            ...
            # Prepare CNN input
            cnn_features = [readings[k] for k in [
